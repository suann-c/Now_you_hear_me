...
#declare type of file output (i.e. filetype.magic)
class FileType:
...
for obj in bpy.data.objects:
    ...
    if not filetype.as_lines:
        ...
        for poly in mesh.polygons:
            ...
            for i in range(0,3):
                ...
                for x in vertex.co: #coordinate of vertex
                    data += struct.pack('f', x) #x is looping through x,y,z and 'f' format str representing float
                if filetype.normal: #pack in triangle normal info too
    ...
    """
    index += struct.pack('I', vertex_count) #packing vertex_count as an unsigned_int
...
#put in ".pn"/".pnc"
blob.write(struct.pack('4s',filetype.magic)) #type (4s == 4 char)
